...
// simple extension to Material-UI buttons to allow for error/success/warning/info colors
...
export const styles = (theme) => {
  const colorThemes = ALLOWED_COLORS.map((colorKey) => ({
    [`text${capitalize(colorKey)}`]: {
    ...
    [`outlined${capitalize(colorKey)}`]: {
      color: theme.palette[colorKey]?.main,
      border: `1px solid ${alpha(theme.palette[colorKey]?.main, 0.5)}`,
      "&:hover": {
